...
class NavigationRootContainer extends React.Component<any, Props, State> {
  ...
  componentDidMount(): void {
    if (this.props.linkingActionMap) {
     ...
     console.log('NavigationRootContainer:navState:8888');
    if (this.props.persistenceKey) {
      AsyncStorage.getItem(this.props.persistenceKey, (err, storedString) => {
        if (err || !storedString) {
          console.log('NavigationRootContainer:navState:33333');
          this.setState({
        ...
        console.log('NavigationRootContainer:navState:6666');
        this.setState({
  ...
  render(): ReactElement {
    console.log('NavigationRootContainer:navState:' + JSON.stringify(this.state.navState));
