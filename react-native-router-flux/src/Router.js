...
class Router extends Component {
  ...
  handleProps(props) {
    ...
    if (this.props.myType != 'intial' && myInitialState && myInitialState.scenes) {
      ...
      let innerScensMap = {};
      Object.keys(scenesMap).map((innerRoute) => {
        if (scenesMap[innerRoute].parent && scenesMap[innerRoute].parent == secondRootRoute) {
          scenesMap[innerRoute].parent = rootRoute;
          innerScensMap[innerRoute]=scenesMap[innerRoute];
        }
      })
      console.log('router-flux:innerScensMap:' + JSON.stringify(innerScensMap));
      ...
      myInitialState.scenes = Object.assign(myInitialState.scenes, innerScensMap);
      secondState.children[0].parent = rootRoute
      myInitialState.children.push(secondState.children[0]);
      myInitialState.index++;
      console.log('router-flux:myInitialState:' + JSON.stringify(myInitialState));
    } else {
